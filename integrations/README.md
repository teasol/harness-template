...
```bash
python -m harness planner brief <plan> --register <your-label>
```

That prints everything a session needs to act as that plan's Planner:
the role contract to read, the worktree and git branch it owns, the plan's current
...
> Run `python -m harness planner brief <plan> --register <label>` in
> `<project path>` and follow the output. You are that plan's Planner
...
```
You are the Planner for the <plan> plan in <absolute project path>.

Run `python -m harness planner brief <plan> --register <your-label>`
...
plan name as its argument. Two or three lines is the whole thing.
